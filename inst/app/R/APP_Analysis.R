Analysis_box_ui1 <- function(id) {
  box(id = "AnalysisBox1", title = p("Overlay Plots"), status = "warning", solidHeader = F,
      ...
      fluidRow(
        ...
        column(1,
               colorSelectorInput(
                 inputId = "mycolor11", label = "Pick a color for trace 11:",
                 choices = c("#88CCEE", "#CC6677", "#DDCC77", "#117733", "#332288", "#AA4499", "#44AA99", "#999933", "#882255", "#661100", "#6699CC", "#888888"),
                 selected = "#6699CC"
               )),
        column(1,
               colorSelectorInput(
                 inputId = "mycolor12", label = "Pick a color for trace 12:",
                 choices = c("#88CCEE", "#CC6677", "#DDCC77", "#117733", "#332288", "#AA4499", "#44AA99", "#999933", "#882255", "#661100", "#6699CC", "#888888"),
                 selected = "#888888"
               )),
      )
...
analysis_server <- function(input, output, session, continue_module, upload_data, ladder_module, peaks_module, metrics_module) {
  ...
  observeEvent(input$normalize, {
    if (!is.null(peaks_module$index_list())) {
      ...
      if (input$normalize == "Highest Peak") {
        ...
        updateNumericInput(session, "ylim2_analysis", value = max(metrics_module$metrics_table()$modal_peak_signal, na.rm = T) + 300)
      }
    }
  })
  ...
  observe({
    if (!is.null(peaks_module$index_list()) && !is.null(input$sample_subset_metrics)) {
      if (!is.null(metrics_module$metrics_table()) && any(!is.na(metrics_module$metrics_table()$modal_peak_repeat))) {
      updateNumericInput(session, "xlim1_analysis", value = min(metrics_module$metrics_table()$modal_peak_repeat, na.rm = T) - 50)
      updateNumericInput(session, "xlim2_analysis", value = max(metrics_module$metrics_table()$modal_peak_repeat, na.rm = T) + 50)
      updateNumericInput(session, "ylim1_analysis", value = -200)
      updateNumericInput(session, "ylim2_analysis", value = max(metrics_module$metrics_table()$modal_peak_signal, na.rm = T) + 300)
      }
      else {
        updateNumericInput(session, "xlim1_analysis", value = 0)
        updateNumericInput(session, "xlim2_analysis", value = 250)
        updateNumericInput(session, "ylim1_analysis", value = -200)
