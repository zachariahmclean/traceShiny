...
metrics_server <- function(input, output, session, continue_module, upload_data, ladder_module, peaks_module) {
  ...
  observe({
    if (!is.null(peaks_module$index_list()) && !is.null(input$sample_subset_metrics)) {
      if (!is.na(peaks_module$index_list()[[input$sample_subset_metrics]]$get_allele_peak()$allele_repeat)) {
      ...
      updateNumericInput(session, "ylim2_metrics", value = peaks_module$index_list()[[input$sample_subset_metrics]]$get_allele_peak()$allele_signal + 300)
        ...
        updateNumericInput(session, "xlim1_metrics", value = 0)
        updateNumericInput(session, "xlim2_metrics", value = 250)
        updateNumericInput(session, "ylim1_metrics", value = -200)
        updateNumericInput(session, "ylim2_metrics", value = 2000)
      }
    }
  })
          ...
          plot_ly(data = data,
            ...
            layout(title = ifelse(is.null(plot_title), peaks_module$index_list()[[input$sample_subset_metrics]]$unique_id, plot_title),
                   ...
                   yaxis = list(title = "Signal",
                                range = ylim),
                   shapes = if(!is.na(debounced_IndexRepeat1())) list(hline(input$peak_threshold*peaks_module$index_list()[[input$sample_subset_metrics]]$.__enclos_env__$private$allele_signal),
                                                                      #vertical line
          ...
          if (!is.null(peak_table$repeats) && !is.null(peak_table$calculated_repeats)) {
            plot_ly(data = data,
              ...
              layout(title = ifelse(is.null(plot_title), peaks_module$index_list()[[input$sample_subset_metrics]]$unique_id, plot_title),
                     ...
                     yaxis = list(title = "Signal",
                                  range = ylim),
                     shapes = if(!is.na(debounced_IndexRepeat2())) list(hline(input$peak_threshold*peaks_module$index_list()[[input$sample_subset_metrics]]$.__enclos_env__$private$allele_signal),
                                                                        #vertical line
